...
-- Browser App: Get saved links from data/links.json
RegisterNUICallback('getLinks', function(data, cb)
    local file = LoadResourceFile(GetCurrentResourceName(), 'data/links.json')
    if file then
        cb(file)
    else
...
-- Listen for server confirmation and forward updated data back to NUI
RegisterNetEvent('redutzu-tablet:client:linksUpdated', function(jsonData)
    SendNUIMessage({
        action = 'linksUpdated',
        data = jsonData
    })
